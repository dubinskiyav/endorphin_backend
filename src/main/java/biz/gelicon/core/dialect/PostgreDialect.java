...
public class PostgreDialect extends BaseDialect implements DBDialect{
    ...
    public String extractColumnNameFromDuplicateMessage(String message) {
        String column = substrToBracket(message, "Ключ \"(");
        if(column==null) {
            column = substrToBracket(message, "Key (");
        };
        return column;
    }

    private String substrToBracket(String message,String keyPhrase) {
    ...
    public String extractTableNameFromForeignKeyMessage(String message) {
        ...
        if(idx<0) {
            ...
            if(idx<0) {
                // вариант сообщения на английском
                keyPhrase = "is still referenced from table \"";
                idx = message.indexOf(keyPhrase);
            }
        }
        if(idx>=0) {
