...
public class RestControllerExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleUnexpectedException(Exception e) {
        ...
        if (e instanceof MemoryDatasetProcessException) {
            errorResponse.setErrorCode(POST_ERROR);
            errorResponse.setCause(e.getCause().getMessage());
            // нарушение уникальности
            if(e.getCause() instanceof DuplicateKeyException) {
                errorResponse.setCause(e.getCause().getMessage());
                processDuplicateError(e.getCause().getMessage(),errorResponse);
            } else;
        }
    ...
    private void processDuplicateError(String message,ErrorResponse errorResponse) {
        processDuplicateError(null,message,errorResponse);
    }

    private void processDuplicateError(Exception ex, String message,ErrorResponse errorResponse) {
        Field fld = null;
        String columnName = DialectFactory.getDialect().extractColumnNameFromDuplicateMessage(message);
        String fldDesc = columnName!=null?columnName:"?";
        if(columnName!=null && ex!=null) {
            String[] fieldsNames = columnName.split(",");
            // есть только одно поле
            if(fieldsNames.length==1) {
                fld = OrmUtils.getField(((PostRecordException)ex).getModelCls(), fieldsNames[0]);
            }
            for (int i = 0; i < fieldsNames.length; i++) {
                fieldsNames[i] = getColumnDesc(fieldsNames[i],((PostRecordException)ex));
            }
            fldDesc = String.join(",",fieldsNames);
        }
